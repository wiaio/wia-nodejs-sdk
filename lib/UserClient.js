...
var UserClient = function(token, opt) {
  ...
  this.subscribeToDeviceEvent = function(deviceKey, eventName, cb) {
    if (!this.mqttClient) {
      ...
      this.mqttClient.on('connect', function() {
        this.subscribe("devices/" + deviceKey + "/events/" + eventName);
        self.subscribeCallbacks["devices/" + deviceKey + "/events/" + eventName] = cb;
      });
    } else {
      this.mqttClient.subscribe("devices/" + deviceKey + "/events/" + eventName);
      this.subscribeCallbacks["devices/" + deviceKey + "/events/" + eventName] = cb;
    }
  }
  ...
  this.subscribeToAllDeviceEvents = function(deviceKey, opt, cb) {
    if (!this.mqttClient) {
      this.initStreamClient();
      this.mqttClient.on('connect', function() {
  ...
  this.sendCommand = function(deviceKey, commandName, commandData, cb) {
    console.log("Send command!");
    if (!this.mqttClient) {
      this.initStreamClient();
      this.mqttClient.on('connect', function() {
        console.log("Publishing " + 'devices/' + deviceKey + '/commands/' + commandName);
