...
var DeviceClient = function(token, opt) {
  ...
  this.publishLog = function(logLevel, logMessage, logData, cb) {
    var publishData = {
        level: logLevel,
        message: logMessage,
        data: logData
    ...
    if (this.options.stream && self.mqttClient) {
      self.mqttClient.publish('device/log', JSON.stringify(publishData),
          { qos: MQTT_CLIENT_QOS, retain: MQTT_CLIENT_RETAIN }, function() {
          if (cb)
              cb();
      });
    } else {
      request.post(this.restProtocol + REST_CLIENT_ENDPOINT + "/logs", {
        body: publishData,
        auth: {
          bearer: token
        },
