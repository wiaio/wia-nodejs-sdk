...
function WiaStream(wia) {
  ...
  this.connect = function() {
    ...
    self.mqttClient.on('connect', function () {
        ...
        self.connected = true;
        self.mqttClient.subscribe('whoami');
        self.emit('connect');
    });
    ...
    self.mqttClient.on('message', function (topic, message) {
      try {
        if (topic === "whoami") {
          if (message) {
            var clientInfo = JSON.parse(message.toString());
            wia.clientInfo = clientInfo

            if (clientInfo.scope === "device" && wia.getApiField('enableCommands')) {
              self.mqttClient.subscribe('devices/' + clientInfo.device.deviceKey + "/commands/run");
            }
          }
        } else if (topic.indexOf('devices') === 0) {
          ...
          if (topicSplit) {
            ...
            if (topicAction.indexOf("events") == 0 &&
              ...
              self.subscribeCallbacks["devices/" + deviceKey + "/events/+"](msgObj);
            } else if (topicAction.indexOf("logs") == 0 &&
                  typeof self.subscribeCallbacks["devices/" + deviceKey + "/logs/+"] === "function") {
              self.subscribeCallbacks["devices/" + deviceKey + "/logs/+"](msgObj);
