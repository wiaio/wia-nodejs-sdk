...
function WiaResourceFunctions(wia) {
  ...
  this.register = function(opt, commandFunc, cb) {
    request.post(wia.getApiUrl() + "functions/register", {
      ...
      headers: wia.getHeaders()
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          wia.stream.subscribe('devices/' + wia.clientInfo.deviceKey + '/functions/' + opt.name + '/call', commandFunc);
          if (cb)
            cb(null, body);
        } else {
          if (cb)
  ...
  this.deregister = function(opt, cb) {
    request.put(wia.getApiUrl() + "functions/deregister", {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        if (wia.clientInfo)
          wia.stream.unsubscribe('devices/' + wia.clientInfo.deviceKey + '/functions/' + opt.name + '/call');
        if (cb)
          cb(null, body);
      } else {
        if (cb)
