...
function WiaResourceEvents(wia) {
  this.publish = function(opt, cb) {
    ...
    if (opt.file) {
      ...
      wia.stream.publish('devices/' + wia.clientInfo.device.id + '/events/' + opt.name, JSON.stringify(opt), cb);
    }
  },
  this.subscribe = function(opt, cb) {
    ...
    if (!opt.name) {
      wia.stream.subscribe('devices/' + opt.device + '/events/+', cb);
    } else {
  ...
  this.unsubscribe = function(opt, cb) {
    ...
    if (!opt.name) {
      wia.stream.unsubscribe('devices/' + opt.device + '/events/+', cb);
    } else {
